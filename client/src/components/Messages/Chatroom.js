...
function Chatroom(props) {
  ...
  function handleDonateItem() {
    setPopup(false);
    const recipient =
      props.arr[0].user_id === user.id
        ? props.arr[0].recipient_id
        : props.arr[0].user_id;
    const newDonation = {
  ...
  const confirmPopup = {
    ...
    buttons: [
      <Button onClick={handleDonateItem} variant="contained">
        Yes, Donate!
      </Button>,
      <Button onClick={() => setPopup(false)} variant="outlined">
        Whoops! No, I do not want to donate this item
      </Button>,
    ],
  };
    ...
    <div>
      <div className="chatroom">
        <Typography variant="h4">
          Your conversation with {targetUsername}
        </Typography>
        <Typography variant="h6">
          Donate one of your items to this user
        </Typography>
      ...
      <Popup
        popupMessage={confirmPopup}
        trigger={popup}
        setPopupTrigger={setPopup}
      />
      <Chatbox
